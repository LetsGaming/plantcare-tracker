/**
 * modules/substrate/infrastructure/MySQLSubstrateRepository.ts
 *
 * V1 improvement: substrateModel had N+1 selectEntityImages calls.
 * V2: images fetched via single JOIN per substrate batch.
 */
...
const BASE_QUERY = `
  ...
  LEFT JOIN substrate_images si ON si.substrate_id = s.id
  LEFT JOIN images img ON img.id = si.image_id
`;

export class MySQLSubstrateRepository implements SubstrateRepository {
  constructor(private readonly pool: Pool) {}

  async findAllPublic(): Promise<SubstrateData[]> {
    const [rows] = await this.pool.query<SubstrateRow[]>(`${BASE_QUERY} WHERE s.is_public = 1`);
    return this.groupRows(rows);
  }

  async findAllByUser(userId: number): Promise<SubstrateData[]> {
    const [rows] = await this.pool.query<SubstrateRow[]>(`${BASE_QUERY} WHERE s.user_id = ?`, [userId]);
    return this.groupRows(rows);
  }

  async findById(id: number): Promise<SubstrateData | null> {
    const [rows] = await this.pool.query<SubstrateRow[]>(`${BASE_QUERY} WHERE s.id = ?`, [id]);
  ...
  async update(id: number, userId: number, fields: { name?: string; isPublic?: boolean }): Promise<boolean> {
    const updates: string[] = [];
    const params: unknown[] = [];
    if (fields.name !== undefined) { updates.push('name = ?'); params.push(fields.name); }
    if (fields.isPublic !== undefined) { updates.push('is_public = ?'); params.push(fields.isPublic ? 1 : 0); }
  ...
  async addComponents(substrateId: number, components: { componentId: number; parts: number }[]): Promise<void> {
    await Promise.all(components.map(({ componentId, parts }) =>
      this.pool.execute(
  ...
  async upsertComponents(substrateId: number, components: { componentId: number; parts: number }[]): Promise<void> {
    await Promise.all(components.map(({ componentId, parts }) =>
      this.pool.execute(
        `INSERT INTO substrate_components (substrate_id, component_id, parts)
         VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE parts = VALUES(parts)`,
        [substrateId, componentId, parseFloat(parts.toString()).toFixed(2)],
      ),
    ));
  }
  ...
  private groupRows(rows: SubstrateRow[]): SubstrateData[] {
    ...
    for (const row of rows) {
      ...
      if (row.image_id && !s.images.find((i) => i.id === row.image_id)) {
        const imageRef: ImageRef = { id: row.image_id, url: row.image_url ?? '', date: row.upload_date ?? '' };
