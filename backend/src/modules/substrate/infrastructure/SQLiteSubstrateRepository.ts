/**
 * modules/substrate/infrastructure/SQLiteSubstrateRepository.ts
 *
 * Key V2 improvement preserved: single JOIN query per batch fetch instead
 * of N+1 selectEntityImages calls from V1.
 *
 * SQLite-specific note:
 *  • MySQL's ON DUPLICATE KEY UPDATE is replaced by INSERT OR REPLACE.
 *    INSERT OR REPLACE deletes the old row and inserts a new one, which
 *    is semantically equivalent for our composite PK (substrate_id, component_id)
 *    and the single column being updated (parts).
 *  • is_public coerced to boolean in groupRows().
 */
...
export class SQLiteSubstrateRepository implements SubstrateRepository {
  ...
  async upsertComponents(substrateId: number, components: { componentId: number; parts: number }[]): Promise<void> {
    // INSERT OR REPLACE is SQLite's equivalent of ON DUPLICATE KEY UPDATE.
    // It deletes the conflicting row (same PK) then inserts the new one.
    transaction(({ execute: exec }) => {
      for (const { componentId, parts } of components) {
        exec(
...
function groupRows(rows: SubstrateRow[]): SubstrateData[] {
