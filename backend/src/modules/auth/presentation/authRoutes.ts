...
export const createAuthRouter = (pool: Pool): Router => {
  ...
  // POST /request-ticket
  router.post('/request-ticket', authenticateToken, (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.post('/logout', (req: Request, res: Response) => {
    ...
    const isHttps = req.secure || req.headers['x-forwarded-proto'] === 'https';
    res.clearCookie('refreshToken', { ...COOKIE_BASE, secure: isHttps, path: refreshCookiePath });
    res.json({ success: true, data: { loggedOut: true } });
  });

  // PUT /update (own profile)
  router.put('/update', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  // PUT /update/:id (admin)
  router.put('/update/:id', authenticateToken, isAdmin, async (req: Request, res: Response, next: NextFunction) => {
    try {
