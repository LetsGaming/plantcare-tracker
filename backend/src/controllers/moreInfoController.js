...
const PLANT_SOURCES = {
  jungleLeaves: async (plantName) => {
    const url = `https://www.jungle-leaves.de/?s=${encodeURIComponent(plantName)}`;
    const data = await fetchData(url);
    if (!data) return null;

    const links = [...new Set(parse(data)
  ...
  rhs: async (plantName) => {
    const url = "https://lwapp-uks-prod-psearch-01.azurewebsites.net/api/v1/plants/search/advanced";
    const payload = { startFrom: 0, pageSize: 20, keywords: plantName, includeAggregation: true };
    ...
    const bestMatch = findBestMatch(plantName, plantLinks.map((p) => p.name));
  ...
  wikipedia: async (plantName) => {
    const url = `https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(plantName)}&format=json`;
    const data = await fetchData(url);
    if (!data || !data.query?.search.length) return null;

    return findBestMatch(
      plantName,
      data.query.search.map((r) => `https://en.wikipedia.org/wiki/${r.title.replace(/ /g, "_")}`)
    );
  },

  gbif: async (plantName) => {
    const url = `https://api.gbif.org/v1/species/search?q=${encodeURIComponent(plantName)}&limit=5`;
    const data = await fetchData(url);
    if (!data || !data.results?.length) return null;

    const bestMatch = findBestMatch(plantName, data.results.map((r) => r.species));
...
const getMoreInfo = async (req, res) => {
  ...
  let cleanedName = plantName.replace(/[^a-zA-Z0-9 ]/g, "").replace(/\s*\([^)]*\)/g, ""); // Clean input
