...
const PLANT_SOURCES = {
  ...
  wikipedia: createSearcher({
    key: "wikipedia",
    isApi: true,
    searchUrl: (q) => `https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${q}&format=json`,
  ...
  gbif: createSearcher({
    ...
    handleApi: (data, query, matchFn) => {
      ...
      const bestName = matchFn(query, data.results.map(r => r.species));
      const result = data.results.find(r => r.species === bestName);
      return result?.nubKey ? `https://www.gbif.org/species/${result.nubKey}` : null;
    }
  }),
...
rhs: createSearcher({
    key: "rhs",
    isApi: true,
    searchUrl: () => "https://lwapp-uks-prod-psearch-01.azurewebsites.net/api/v1/plants/search/advanced",
    ...
    handleApi: (data, query, matchFn) => {
      ...
      const bestName = matchFn(query, plantLinks.map(p => p.name));
      const bestPlant = plantLinks.find(p => p.name === bestName);
      return bestPlant ? `https://www.rhs.org.uk/plants/${bestPlant.id}/${bestPlant.name.replace(/ /g, "-").toLowerCase()}/details` : null;
    }
  })
};
