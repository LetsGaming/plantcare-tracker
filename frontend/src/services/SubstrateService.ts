...
const SubstrateService = {
  ...
  async getSubstrates(forceUpdate: boolean = false): Promise<any[]> {
    ...
    const cachedData = await storageService.get<{ substrates: any[], timestamp: number }>(CACHE_KEY_SUBSTRATES);
    ...
    try {
      const substrates = await ApiUtils.get<any[]>(SUBSTRATES_ENDPOINT);

      // Store the substrates and current timestamp in storage
      await storageService.set(CACHE_KEY_SUBSTRATES, { substrates, timestamp: Date.now() });
   ...
   */
  async getSubstrateById(id: number, forceUpdate: boolean = false): Promise<any> {
    const cachedData = await storageService.get<{ substrates: any[], timestamp: number }>(CACHE_KEY_SUBSTRATES);

    // Check if cached data exists and is not expired
    if (cachedData && !isCacheExpired(cachedData.timestamp)) {
      const substrate = cachedData.substrates.find(s => s.substrate_id === id);
      if (substrate) {
  ...
  async addSubstrate(substrateData: any): Promise<any> {
    try {
      const response = await ApiUtils.post<any, any>(SUBSTRATES_ENDPOINT, substrateData);
  ...
  async updateSubstrate(id: number, substrateData: any): Promise<any> {
    try {
      const response = await ApiUtils.put<any, any>(`${SUBSTRATES_ENDPOINT}/${id}`, substrateData);
